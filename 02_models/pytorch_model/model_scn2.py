...
class ConvPoolBlock(nn.Module):
    def __init__(self, input_channels : int, output_channels : int) -> None:
        ...
        self.block = nn.Sequential(
            ConvBlock2D(input_channels,   output_channels, kernel_size = 3, padding = 1),
            ConvBlock2D(output_channels,  output_channels, kernel_size = 1, padding = 0),
...
class ExtendedSimpleCNN2D(nn.Module):
    ...
    def forward(self, x : torch.Tensor) -> torch.Tensor:
        ...
        # Utilisation de .view pour aplatir les dimensions [batch_size, 256, 1, 1] en [batch_size, 256]
        x = x.view(x.size(0), -1)
